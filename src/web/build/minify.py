...
if not (has_html_minifier and has_cleancss and has_terser):
    print("Warning: Minifiers not found — falling back to plain copy.", file=sys.stderr)
    ...
    print(
        "For smaller assets install: npm install -g html-minifier-terser clean-css-cli terser",
        file=sys.stderr,
    )
    # Plain-copy fallback so the LittleFS pipeline still produces a usable
    # web UI without the npm minifier toolchain.
    for f in sorted(SRC.rglob("*")):
        ...
        shutil.copy(f, out)
    print("Web assets copied unminified")
else:
